    ...
    public class AuditValidator : AbstractValidator
    {
        public override void Validate(ExtendedDataEntity[] dataEntities, ValidateContext validateContext, Context ctx)
        {
            ...
            foreach (var data in dataEntities)
            {
                ...
                //拆卸：Dassembly 组装：Assembly
                var affairType = Convert.ToString(data["AffairType"]);

                //成品：ProductEntity  子件表：STK_ASSEMBLYSUBITEM
                var entrys = data["ProductEntity"] as DynamicObjectCollection;
                long orgId = Convert.ToInt64(dynamicObject["StockOrgId_Id"].ToString());
                foreach (var item in entrys)
                {
                    //拆卸取成品
                    if (affairType.EqualsIgnoreCase("Dassembly"))
                    {
                        detList.Add(new GroupStock
                        {
                            ...
                            Qty = decimal.Parse(item["FQty"].ToString()),
                            DataEntity = data
                        });
                    }
                    else if (affairType.EqualsIgnoreCase("Assembly"))   //组装取子件
                    {
                        var entrys2 = item["STK_ASSEMBLYSUBITEM"] as DynamicObjectCollection;
                        foreach (var item2 in entrys2)
                        {
                            detList.Add(new GroupStock
                            {
                                BillNO = Convert.ToString(data["BILLNO"]),
                                StockOrgId = orgId,
                                MasterID = Convert.ToInt64(((DynamicObject)item2["MaterialIDSETY"])["msterID"]),
                                MaterialCode = Convert.ToString(((DynamicObject)item2["MaterialIDSETY"])["Number"]),
                                StockID = Convert.ToInt64(((DynamicObject)item2["FStockIDSETY"])["id"]),
                                StockName = Convert.ToString(((DynamicObject)item2["FStockIDSETY"])["Name"]),
                                Qty = decimal.Parse(item2["FQtySETY"].ToString()),
                                DataEntity = data
                            });
                        }
