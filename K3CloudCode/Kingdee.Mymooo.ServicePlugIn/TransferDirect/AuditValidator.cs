    ...
    public class AuditValidator : AbstractValidator
    {
        public override void Validate(ExtendedDataEntity[] dataEntities, ValidateContext validateContext, Context ctx)
        {
            ...
            foreach (var data in dataEntities)
            {
                ...
                var BillTypeID =Convert.ToString(dynamicObject["BillTypeID_Id"]);
                if (BillTypeID== "005056941128823c11e323525db18103")
                {
                    continue;
                }

				var entrys = data["TransferDirectEntry"] as DynamicObjectCollection;
                long orgId = Convert.ToInt64(dynamicObject["StockOutOrgId_Id"].ToString());
                foreach (var item in entrys)
                {
                    detList.Add(new GroupStock
                    {
                        ...
                        StockID = Convert.ToInt64(((DynamicObject)item["SrcStockId"])["id"]),
                        StockName = Convert.ToString(((DynamicObject)item["SrcStockId"])["Name"]),
