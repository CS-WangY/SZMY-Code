...
namespace Kingdee.Mymooo.ServicePlugIn.SalesBill
{
    ...
    /// 整单关闭验证
    /// </summary>
    public class CompleteClosedValidator : AbstractValidator
    {
        public override void Validate(ExtendedDataEntity[] dataEntities, ValidateContext validateContext, Context ctx)
        {
            foreach (var data in dataEntities)
            {
                ...
                var sql = $@"select top 1 t1.FBILLNO from  T_SAL_DELIVERYNOTICE t1 
                                ...
                                and t3.FSBILLID='{billid}'  and t2.FSumOutQty=0";
                ...
                if (!string.IsNullOrWhiteSpace(retStr))
                {
                    validateContext.AddError(data, new ValidationErrorInfo(
                                    ...
                                    data.RowIndex,
                                    data["Id"].ToString(),
                                    string.Format("单据编号[{0}]已存在正常未出库的发货通知单[{1}]，不能关闭销售订单", data["BillNo"], retStr),
                                    $"整单关闭[{data["BillNo"]}]",
