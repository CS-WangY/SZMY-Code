...
namespace Kingdee.Mymooo.Business.PlugIn.StockManagement
{
    ...
    /// 云仓储撤回入库单接口
    /// </summary>
    public class CloudStockRevocationStockInBusiness : IMessageExecute
    {
        public ResponseMessage<dynamic> Execute(Context ctx, string message)
        {
            ...
            List<CloudStockRevocationStockInRequest> requests = JsonConvertUtils.DeserializeObject<List<CloudStockRevocationStockInRequest>>(message);
            if (requests.Count == 0)
            {
                response.Code = ResponseCode.ModelError;
                response.Message = "同步到云仓储撤回入库单信息不能为空";
                return response;
            }
            var requestData = JsonConvertUtils.SerializeObject(requests);
            var cwResult = WarehouseApiRequest.Request(WarehouseApiRequest.CloudStockUrl, WarehouseApiRequest.CloudStockToken, "/api/goods/unshelved", requestData, "MYMO", "DELETE");
