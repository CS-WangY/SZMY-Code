...
using static Kingdee.Mymooo.Business.PlugIn.SalesManagement.DnUrgentShipmentList;
using Kingdee.Mymooo.Core.Common;

namespace Kingdee.Mymooo.Business.PlugIn.SalesManagement
{
    [Description("发货通知单加急发货申请列表插件"), HotUpdate]
    public class DnUrgentShipmentList : AbstractDynamicFormPlugIn
    {
        public override void AfterBindData(EventArgs e)
        {
            ...
            {
                var sql = $@"/*dialect*/select CustName,Warehouse,SUM(AMOUNT) FBILLAMOUNT,Count(FENTRYID) OutCount from (
                            select  t3.FNAME CustName,case when t2.FSUPPLYTARGETORGID=7401782 then '大岭山非标件仓库'
							                            ...
							                            ,(t2.FQTY*ISNULL(t6.FTAXNETPRICE,0)) AMOUNT,t2.FENTRYID
                ...
                foreach (var data in datas)
                {
                    ...
                    this.Model.SetValue("F_PENY_OutCount", Convert.ToInt32(data["OutCount"]));
                    this.Model.SetValue("F_PENY_DnNo", dnNos);
                    this.Model.SetValue("F_PENY_Warehouse", data["Warehouse"]);
                    this.Model.SetValue("F_PENY_DnAddr", GetCustAddress(this.Context, dnNos.Split('、')[0]));
                    this.View.UpdateView("F_PENY_ApplyDate");
                    this.View.UpdateView("F_PENY_CustName");
                    this.View.UpdateView("F_PENY_OrderAmount");
                    this.View.UpdateView("F_PENY_OutCount");
                    this.View.UpdateView("F_PENY_DnNo");
                    this.View.UpdateView("F_PENY_Warehouse");
                    this.View.UpdateView("F_PENY_DnAddr");
                }
            }
        }
        ...
        public override void AfterButtonClick(AfterButtonClickEventArgs e)
        {
            ...
            if (e.Key.ToUpper().Equals("FOK"))
            {
                try
                {
                    ...
                    var warehouse = Convert.ToString(this.View.Model.GetValue("F_PENY_Warehouse"));
                    var dnAddr = Convert.ToString(this.View.Model.GetValue("F_PENY_DnAddr"));
                    var outCount = Convert.ToInt32(this.View.Model.GetValue("F_PENY_OutCount"));
                    var urgentDeliveryDate = Convert.ToDateTime(this.View.Model.GetValue("F_PENY_UrgentDeliveryDate"));
                    ...
                    if (urgentDeliveryDate == null || Convert.ToString(urgentDeliveryDate).Contains("0001-01-01"))
                    {
                        this.View.ShowErrMessage("加急出库日期不能为空");
                        return;
                    }
                    if (urgentDeliveryDate < applyDate)
                    {
                        this.View.ShowErrMessage("加急出库日期不能小于当前日期");
                        return;
                    }
                    ...
                    //执行调用审批流
                    K3cloudDnUrgentShipmentRequest request = new K3cloudDnUrgentShipmentRequest();
                    request.DnNo = dnNos;
                    request.CustName = custName;
                    request.Warehouse = warehouse;
                    request.OrderAmount = orderAmount;
                    request.OutCount = outCount;
                    request.UrgentDeliveryDate = urgentDeliveryDate;
                    request.Remarks = remarks;
                    request.ApplyDate = applyDate;
                    request.Creator_userid = wxUserCode;
                    request.SendRabbitCode = "mymooo-weixin-Approval-UpDnUrgentShipmentState-" + ApigatewayUtils.ApigatewayConfig.EnvCode;
                    ...
                    var result = ApigatewayUtils.InvokePostWebService($"k3cloudapi/{ApigatewayUtils.ApigatewayConfig.EnvCode}/Approval/K3cloudDnUrgentShipmentApproval", JsonConvertUtils.SerializeObject(request));
                    ...
                    List<SqlParam> pars = new List<SqlParam>() {
                        ...
                        new SqlParam("@FDnAddr", KDDbType.String, dnAddr),
                        new SqlParam("@FOrderAmount", KDDbType.String, orderAmount),
                        new SqlParam("@FOutCount", KDDbType.Int32, outCount),
                        new SqlParam("@FUrgentDeliveryDate", KDDbType.Date, urgentDeliveryDate),
                    ...
                    var sql = $@"INSERT INTO T_SAL_DnUrgentShipment
                                   (
                                    FApplyDate
                                   ,FApprovalNo
                                   ,FDnNo
        ...
        public class K3cloudDnUrgentShipmentRequest
        {
            /// <summary>
            /// DN单号
            /// </summary>
        ...
        public class DnUSCloudWarehouseMessage
        {
            /// <summary>
            /// 是否成功
            /// </summary>
            ...
            public List<CloudStockData> Data { get; set; }
        }

        public class CloudStockData
        {
    ...
    [Description("发货通知单-打开加急发货申请列表插件"), HotUpdate]
    public class OpenDnUrgentShipmentList : AbstractListPlugIn
    {
        public override void BarItemClick(BarItemClickEventArgs e)
        {
            ...
            switch (e.BarItemKey)
            {
                case "PENY_tbDnUrgentShipmentList":
                    ...
                    foreach (string key in listcoll.GroupBy(g => g.BillNo).Select(s => s.Key))
                    {
                        ...
                        //}
                        //判断是否已经存在审批中的任务
                        sql = $@"/*dialect*/select count(1) from  T_SAL_DnUrgentShipment where FDnNo like '%{key}%' and FSpStatus=1";
                        if (DBServiceHelper.ExecuteScalar<int>(this.Context, sql, 0) > 0)
                        {
                            this.View.ShowErrMessage($"订单{key}已存在审批中的加急发货申请");
                            return;
                        }
                        //判断是否已经存在已审批的任务
                        sql = $@"/*dialect*/select count(1) from  T_SAL_DnUrgentShipment where FDnNo like '%{key}%' and FSpStatus=2";
                        if (DBServiceHelper.ExecuteScalar<int>(this.Context, sql, 0) > 0)
                        {
                            this.View.ShowErrMessage($"订单{key}已存在已审批的加急发货申请");
                            return;
                        }
                        dnNos.Add(key);
                    }
                    //判断云存储
                    var cwResult = WarehouseApiRequest.RequestData(WarehouseApiRequest.CloudStockUrl, WarehouseApiRequest.CloudStockToken, "/api/goods/DnNoIsExist", JsonConvertUtils.SerializeObject(dnNos));
                    var returnInfo = JsonConvertUtils.DeserializeObject<DnUSCloudWarehouseMessage>(cwResult);
                    if (!returnInfo.IsSuccess)
                    {
                        this.View.ShowErrMessage($"调用云存储错误：" + returnInfo.Message);
                        return;
                    }
                    var errList = returnInfo.Data.Where(x => x.IsExist == false).Select(s => s.ExWarehouseOrderNumber).ToList();
                    if (errList.Count > 0)
                    {
                        this.View.ShowErrMessage($"发货通知单【{string.Join("、", errList)}】还没有同步到云存储，请等待。");
                        return;
                    }
                    ...
                    para.FormId = "PENY_DnUrgentShipmentList";
    ...
    [Description("发货通知单表单-打开加急发货申请列表插件"), HotUpdate]
    public class OpenDnUrgentShipment : AbstractDynamicFormPlugIn
    {
        public override void BarItemClick(BarItemClickEventArgs e)
        {
            base.BarItemClick(e);
            ...
            switch (e.BarItemKey)
            {
                case "PENY_tbDnUrgentShipment":
                    ...
                    //}
                    //判断是否已经存在审批中的任务
                    sql = $@"/*dialect*/select count(1) from  T_SAL_DnUrgentShipment where  FDnNo like '%{orderNo}%' and FSpStatus=1";
                    if (DBServiceHelper.ExecuteScalar<int>(this.Context, sql, 0) > 0)
                    {
                        this.View.ShowErrMessage($"订单{orderNo}已存在审批中的加急发货申请");
                        return;
                    }
                    //判断是否已经存在已审批的任务
                    sql = $@"/*dialect*/select count(1) from  T_SAL_DnUrgentShipment where  FDnNo like '%{orderNo}%' and FSpStatus=2";
                    if (DBServiceHelper.ExecuteScalar<int>(this.Context, sql, 0) > 0)
                    {
                        this.View.ShowErrMessage($"订单{orderNo}已存在已审批的加急发货申请");
                        return;
                    }
                    //判断云存储
                    var cwResult = WarehouseApiRequest.RequestData(WarehouseApiRequest.CloudStockUrl, WarehouseApiRequest.CloudStockToken, "/api/goods/DnNoIsExist", JsonConvertUtils.SerializeObject(new List<string>() { orderNo }));
                    var returnInfo = JsonConvertUtils.DeserializeObject<DnUSCloudWarehouseMessage>(cwResult);
                    if (!returnInfo.IsSuccess)
                    {
                        this.View.ShowErrMessage($"调用云存储错误：" + returnInfo.Message);
