...
{
    [Description("物流方式变更申请报表"), HotUpdate]
    public class SalesDnLogisticsChangesRpt : SysReportBaseService
    {
        ...
        public override ReportTitles GetReportTitles(IRptParams filter)
        {
            ...
            if (customFilter != null)
            {
                ...
                reportTitles.AddTitle("F_PENY_LogisticsType", Convert.ToString(customFilter["F_Filter_LogisticsType"]) == "" ? "全部" : Convert.ToString(customFilter["F_Filter_LogisticsType"]));
            }
            return reportTitles;
        }
        ...
        public override void BuilderReportSqlAndTempTable(IRptParams filter, string tableName)
        {
            ...
            //物流方式
            if (!string.IsNullOrWhiteSpace(Convert.ToString(customFilter["F_Filter_LogisticsType"])))
            {
                where += $" and FLogisticsType='{Convert.ToString(customFilter["F_Filter_LogisticsType"]).Trim()}' ";
            }
            ...
            string sql = $@"/*dialect*/select {KSQL_SEQ},*,
                            ...
                            into {tableName}
                            from T_SAL_DnLogisticsChanges {where} ";
